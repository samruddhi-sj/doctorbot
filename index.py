...
def symptoms():
    ...
    #call and execute query
    cur = con.cursor()
    cur.execute("SELECT DISTINCT symptom FROM symptoms")
    #fetch all the symptomes
    symptom = cur.fetchall()
    #using request we will get this form filled in the session getting gender and age
...
#if one of the answer is selected then it will be appended to the answer to previous symptomes
...
def chooseOne():
    ...
    if choosen[0] == '1':
        ...
        symptoms.append(choosen)
    #if not choosen the question then repeat question, it will ask 2 question again
    myQuestions1,myQuestions2 = Question_Maker.fillter(symptoms)
    if len(myQuestions2) > 0:
        global Q1
        Q1=myQuestions1
